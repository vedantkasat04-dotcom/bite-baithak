...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#1a0f0a]/95 backdrop-blur-sm border-b border-[#d4af6a]/10' : 'bg-transparent'}`}>
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-8">
          ...
          <button className="text-[#f5e6d3] opacity-60 hover:opacity-100 text-xs tracking-[0.2em] transition-opacity">CART (0)</button>
        </div>
      </div>
    </nav>
  )
